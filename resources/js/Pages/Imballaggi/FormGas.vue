<template>
  <AppLayout>
    <div class="page-header">
      <h1 class="page-title">{{ isEdit ? 'Modifica Lotto Gas' : 'Nuovo Lotto Gas' }}</h1>
      <Link href="/imballaggi?tab=gas">
    ...
    <form @submit.prevent="submit" class="form-card">
      <section class="form-section">
        <h2 class="section-title">Gas tecnico / MAP</h2>
        <div class="form-grid">

          <div class="field field-full">
            <label>Fornitore *</label>
            <Select v-model="form.fornitore_id" :options="fornitori" option-label="ragione_sociale" option-value="id"
                    placeholder="Seleziona fornitore..." :invalid="!!form.errors.fornitore_id" filter fluid />
            <small class="error">{{ form.errors.fornitore_id }}</small>
          </div>

          <div class="field">
            <label>Componente *</label>
            <InputText v-model="form.componente" :invalid="!!form.errors.componente" fluid placeholder="es. TRESARIS NC30 bombola grande" />
          ...
          <div class="field">
            <label>U.M.</label>
            <Select v-model="form.um" :options="umOptions" option-label="label" option-value="value" placeholder="—" fluid />
          </div>
...
const umOptions = [
  { label: 'Bombole', value: 'bomb' }, { label: 'Kg', value: 'kg' },
  { label: 'Lt', value: 'lt' }, { label: 'm³', value: 'm3' }, { label: 'Pz', value: 'pz' },
];
...
function submit() {
  ...
  if (isEdit.value) {
    form.transform(() => payload).put(`/imballaggi/gas/${props.lotto.id}`);
  } else {
    form.transform(() => payload).post('/imballaggi/gas');
  }
}
</script>

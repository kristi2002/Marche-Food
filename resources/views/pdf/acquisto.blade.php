...
<head>
<meta charset="utf-8">
<title>Acquisto — {{ $acquisto->numero_documento }}</title>
<style>
  * { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .section-title { font-size: 9px; font-weight: 700; text-transform: uppercase; letter-spacing: 0.1em; color: #2e6b57; background: #f0fdf4; border-left: 3px solid #2e6b57; padding: 4px 8px; margin-bottom: 6px; }
...
<div class="header">
  <div class="brand">
    ...
    <p>Registro Acquisti — Tracciabilità HACCP</p>
  </div>
  <div class="doc-info">
    <div class="num">{{ $acquisto->tipo_documento }} N° {{ $acquisto->numero_documento }}</div>
...
<div class="meta-grid">
  <div class="meta-box">
    ...
    <div class="value">{{ $acquisto->fornitore?->ragione_sociale ?? '—' }}</div>
  </div>
  <div class="meta-box">
    <div class="label">Data Documento</div>
    <div class="value">{{ \Carbon\Carbon::parse($acquisto->data_documento)->format('d/m/Y') }}</div>
  </div>
  <div class="meta-box">
    <div class="label">Tipo</div>
    <div class="value">{{ $acquisto->tipo_documento }}</div>
  </div>
  <div class="meta-box">
    <div class="label">Conto Terzi</div>
    <div class="value">{{ $acquisto->is_conto_terzi ? 'Sì' : 'No' }}</div>
  </div>
</div>
  ...
  <tbody>
    @forelse($acquisto->righe as $r)
      <tr>
        ...
        <td>{{ $r->data_in ? \Carbon\Carbon::parse($r->data_in)->format('d/m/Y') : '—' }}</td>
      </tr>
    @empty
...
<div style="margin-top:14px">
  ...
  <p style="font-size:10px">{{ $acquisto->note }}</p>
</div>
@endif
